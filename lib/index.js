(function() {
  var EOL, Q, amdBundler, coffee, compile, compileAmd, compileCoffee, compileCss, compileExtendFile, compileJs, compileLess, compileSass, extend, extendCache, fs, getParams, getProperty, gutil, less, path, replaceProperties, sass, through, _;
  ...
  extend = function(file, baseFile, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      extendFilePath = '';
      content.replace(/<!--\s*extend\s+(['"])([^'"]+)\.(base\.html)\1\s*-->/mg, function(full, quote, extendName, ext) {
        return extendFilePath = path.resolve(fileDir, extendName + '.' + ext);
      });
      if (extendFilePath) {
