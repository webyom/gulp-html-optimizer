(function() {
  var CleanCSS, EOL, PluginError, Q, UglifyJS, Vinyl, _, amdBundler, coffee, compile, compileAmd, compileBabel, compileCoffee, compileCss, compileExtendFile, compileJs, compileLess, compileSass, compileTs, cssBase64img, cssSprite, extend, extendCache, fs, getDefaultValue, getParams, getProperty, gulpCssSprite, htmlBase64img, interpolateTemplate, less, minifyCSS, minifyJS, path, replaceProperties, safeAccess, sass, sus, through, ts;
  ...
  compileCoffee = function(file, plainId, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      coffeeStream.pipe(through.obj(function(file, enc, next) {
        file.contents = Buffer.from([plainId ? trace + '<script type="text/html" id="' + plainId + '">' : trace + '<script>', minifyJS(file.contents.toString(), file, opt), '</script>'].join(EOL));
        resolve(file);
        return next();
      }));
  ...
  compileTs = function(file, plainId, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      file.contents = Buffer.from([plainId ? trace + '<script type="text/html" id="' + plainId + '">' : trace + '<script>', minifyJS(content.outputText, file, opt), '</script>'].join(EOL));
      return resolve(file);
    });
  };
  ...
  compileBabel = function(file, attrLeft, attrRight, opt) {
    return Q.Promise(function(resolve, reject) {
      return opt.babel(file).then(function(file) {
        ...
        file.contents = Buffer.from(['<script' + attrLeft + attrRight + '>', minifyJS(file.contents.toString(), file, opt), '</script>'].join(EOL));
        return resolve(file);
      }, reject);
    });
  };
  ...
  compileAmd = function(file, baseFile, baseDir, params, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      return amdBundler.bundle(file, {
        ...
        var define, err, exp, factory, mod, outPath, processDefQueue, src;
        if (params.render) {
          ...
          if (params.inline === 'yes') {
            file.contents = Buffer.from([params.plainId ? trace + '<script type="text/html" id="' + params.plainId + '">' : trace + '<script>', minifyJS(file.contents.toString() + EOL + processDefQueue, file, opt), '</script>'].join(EOL));
          } else {
            if (params.out) {
