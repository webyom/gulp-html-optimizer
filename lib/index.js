(function() {
  var EOL, Q, amdBundler, coffee, compile, compileAmd, compileCoffee, compileCss, compileJs, compileLess, fs, getParams, getProperty, gutil, less, path, replaceProperties, through;
  ...
  compileAmd = function(file, baseFile, baseDir, plainId, opt) {
    return Q.Promise(function(resolve, reject) {
      var trace;
      if (opt.trace) {
        trace = '<!-- trace:' + path.relative(process.cwd(), file.path) + ' -->' + EOL;
  ...
  getParams = function(params) {
    ...
    params.forEach(function(param) {
      ...
      m = param.match(/([\w\-]+)=(['"])([^'"]+)\2/);
      if (m) {
        key = m[1].replace(/\-(\w)/g, function(full, w) {
  ...
  compile = function(file, baseFile, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      content = content.replace(/<!--\s*require-base-dir\s+(['"])([^'"]+)\1\s*-->/mg, function(full, quote, base) {
        baseDir = base;
        return '';
      }).replace(/<!--\s*include\s+(['"])([^'"]+)\.(inc\.html|less|coffee|css|js)\1\s*(.*?)\s*-->/mg, function(full, quote, incName, ext, params) {
