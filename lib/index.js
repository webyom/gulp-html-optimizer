  ...
  compileCoffee = function(file, plainId, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      coffeeStream.pipe(through.obj(function(file, enc, next) {
        ...
        file.contents = Buffer.from([plainId ? trace + '<script type="text/html" id="' + plainId + '">' : trace + '<script>', minifyJS(content, file, opt), '</script>'].join(EOL));
        resolve(file);
        return next();
      }));
  ...
  compileBabel = function(file, attrLeft, attrRight, opt) {
    return Q.Promise(function(resolve, reject) {
      return opt.babel(file).then(function(file) {
        ...
        file.contents = Buffer.from(['<script' + attrLeft + attrRight + '>', minifyJS(content, file, opt), '</script>'].join(EOL));
        return resolve(file);
      }, reject);
    });
  };
      ...
      return amdBundler.bundle(file, {
        ...
        if (params.render) {
          ...
          if (params.inline === 'yes') {
            file.contents = Buffer.from([params.plainId ? trace + '<script type="text/html" id="' + params.plainId + '">' : trace + '<script>', minifyJS(content + EOL + processDefQueue, file, opt), '</script>'].join(EOL));
          } else {
            if (params.out) {
