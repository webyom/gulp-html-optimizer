      ...
      return amdBundler.bundle(file, {
        ...
        if (params.render) {
          ...
          if (params.out) {
            ...
            file.contents = new Buffer([params.plainId ? trace + '<script type="text/html" id="' + params.plainId + '">' : trace + '<script type="text/javascript">', minifyJS(file.contents.toString() + EOL + processDefQueue, file, opt), '</script>'].join(EOL));
          }
        }
        return resolve(file);
      }, function(err) {
