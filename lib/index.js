(function() {
  var EOL, PluginError, Q, Vinyl, _, amdBundler, coffee, compile, compileAmd, compileBabel, compileCoffee, compileCss, compileExtendFile, compileJs, compileLess, compileSass, cssBase64img, cssSprite, extend, extendCache, fs, getParams, getProperty, gulpCssSprite, htmlBase64img, less, path, replaceProperties, sass, sus, through;
  ...
  compileBabel = function(file, attrLeft, attrRight, opt) {
    return Q.Promise(function(resolve, reject) {
      return opt.babel(file).then(function(file) {
        file.contents = new Buffer(['<script ' + attrLeft + 'type="text/javascript"' + attrRight + '>', file.contents.toString(), '</script>'].join(EOL));
        return resolve(file);
      }, reject);
    });
  };
  ...
  compileAmd = function(file, baseFile, baseDir, params, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      return amdBundler.bundle(file, {
        ...
        var define, exp, factory, mod, outPath, processDefQueue, ref, ref1, src;
        if (params.render) {
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      if (opt.processRequire !== false) {
        content = content.replace(/<!--\s*require\s+(['"])([^'"]+)\1\s*(.*?)\s*-->/mg, function(full, quote, amdName, params) {
          ...
          params = getParams(params);
          if (opt.processRequire === 'render' && !params.render) {
            return full;
          }
          asyncMark = '<INC_PROCESS_ASYNC_MARK_' + asyncList.length + '>';
