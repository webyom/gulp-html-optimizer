(function() {
      ...
      lessStream.pipe(through.obj(function(file, enc, next) {
        return Q.Promise(function(resolve, reject) {
          ...
          return cssSprite(content, file.path, opt).then(function(content) {
            ...
            file.contents = new Buffer([trace + '<style type="text/css">', minifyCSS(content, file, opt), '</style>'].join(EOL));
            resolve(file);
            return next();
          }, function(err) {
            return reject(err);
  ...
  compileCoffee = function(file, plainId, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      coffeeStream.pipe(through.obj(function(file, enc, next) {
        file.contents = new Buffer([plainId ? trace + '<script type="text/html" id="' + plainId + '">' : trace + '<script type="text/javascript">', minifyJS(file.contents.toString(), file, opt), '</script>'].join(EOL));
        resolve(file);
        return next();
      }));
  ...
  compileBabel = function(file, attrLeft, attrRight, opt) {
    return Q.Promise(function(resolve, reject) {
      return opt.babel(file).then(function(file) {
        file.contents = new Buffer(['<script ' + attrLeft + 'type="text/javascript"' + attrRight + '>', minifyJS(file.contents.toString(), file, opt), '</script>'].join(EOL));
        return resolve(file);
      }, reject);
    });
  };
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      return amdBundler.bundle(file, {
        ...
        if (params.render) {
          ...
          if ((ref = params.process) === 'no' || ref === 'false') {
            processDefQueue = '';
          ...
          if (params.out) {
            if ((ref1 = params.out) === 'yes' || ref1 === 'true' || ref1 === '1') {
            ...
            file.contents = new Buffer([params.plainId ? trace + '<script type="text/html" id="' + params.plainId + '">' : trace + '<script type="text/javascript">', minifyJS(file.contents.toString(), file, opt), processDefQueue, '</script>'].join(EOL));
          }
        }
        return resolve(file);
      }, function(err) {
  ...
  compile = function(file, baseFile, properties, opt) {
    return Q.Promise(function(resolve, reject) {
      ...
      content = content.replace(/<!--\s*include\s+(['"])([^'"]+)\.(less|scss|es6|coffee|css|js|inc\.[^'"]+)\1\s*(.*?)\s*-->/mg, function(full, quote, incName, ext, params) {
